...
The complete technical record for this project: how Torchlight II's two
container formats were reverse-engineered, every tool built to read them, what
is in the archive, and what the item database does with it.

**[README.md](README.md) is the short version** — start there if you just want
to know what this is and how to build it. This file is the long one, and it is
long on purpose: it is the handoff document. Every non-obvious decision, every
dead end worth not repeating, and every number the site displays is argued out
here, because the alternative is re-deriving it.

Read it before changing anything under `src\`. Sections 1–6 are the extraction
work; section 7 is the site built on top of it.
